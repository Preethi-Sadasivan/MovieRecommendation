...
def load_data(file_path="movierecommendation.csv"):
...
def handle_ai(name, movies_df, genres):
    ...
    processing_animation()
    recs = recommend_movies(movies_df, genre=genre, mood=mood, rating=rating, top_n=5)
...
def main():
    ...
    movies_df = load_data('movierecommendation.csv') # Load the dataset
    genres = list_genres(movies_df) # Get the unique genres.
    tfidf = TfidfVectorizer(stop_words='english')
    tfidf_matrix = tfidf.fit_transform(movies_df['combined_features'])
    cosine_sim = cosine_similarity(tfidf_matrix, tfidf_matrix)
    # Add cosine_sim to the movies_df
    movies_df['cosine_sim'] = list(cosine_sim)

    name = input(Fore.CYAN + "Hello! What's your name? ").strip().title()
    handle_ai(name, movies_df, genres)
